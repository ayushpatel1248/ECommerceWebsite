...
ProductStockController.productStockEdit = async (req, res) => {
  const { authorization } = req.headers;
  console.log("authorization is ", authorization)
  const _id = verifyAdminAuth(authorization)

  const { productId, editNo } = req.body;


  if (_id) {
    const result = await ProductsService.productStockEdit(_id, productId, editNo)
    res.send(result)
  }
  else {
    res.send({
