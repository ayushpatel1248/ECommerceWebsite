...
const AllProductsPage = () => {
    ...
    useEffect(() => {
        dispatch(fetchProductData(1, 12))
    }, [])

    const handlePagination = (pageNumber) => {
        dispatch(fetchProductData(pageNumber, 12))
    }
    ...
    return (
        <div>
            ...
            {getProductData.productData.length < 1 ? <Loader /> : <div className='card-product-parent'>
                {getProductData.productData.map((el) => {
                    return <div class="card-product card-product-allproduct">
                        ...
                        <div class="card-info-product">
                            <p class="text-title-product">{el.name.length > 12 ? el.name.substr(0, 12) + '...' : el.name}</p>
